...
from expo_blog import app, bcrypt, db, mail
from expo_blog.forms import RegistrationForm, LoginForm, AccountUpdateForm, PostForm, RequestResetForm, ResetPasswordForm, CommentForm
from expo_blog.models import User, Post, Comment
...
def index():
    if current_user.is_authenticated:
        ...
        return render_template('index.html', posts=posts, avatar=avatar)
    page = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.date_posted.desc()).paginate(page=page, per_page=5)
...
def post(post_id):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            comment = Comment(body=form.body.data, article=post, comm_author=current_user)
            db.session.add(comment)
            db.session.commit()
            flash('Ваш комментарий добавлен!', 'success')
            return redirect(url_for('post', post_id=post.id))
    return render_template('post.html', post=post, title='Ваш комментарий', form=form)
...
def send_reset_mail(user):
    ...
    msg = Message('Сброс пароля',sender ='blogexpo001@gmail.com', recipients=[user.email])
